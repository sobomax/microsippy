...
static int
parse_u32(const char *arg, uint32_t minv, uint32_t maxv, uint32_t *outp)
{
    char *endp;
    unsigned long v;
...
static int
parse_target(const char *ip, uint16_t port, struct sockaddr_storage *ss,
  socklen_t *slenp, struct usipy_sip_tm_addr *targetp)
{
    struct in_addr v4;
#ifdef AF_INET6
    struct in6_addr v6;
...
static int
format_uri_host(char *buf, size_t len, const struct usipy_sip_tm_addr *targetp)
{
    int blen;
...
main(int argc, char **argv)
{
    ...
    while (argi < argc && strncmp(argv[argi], "--", 2) == 0) {
        ...
        if (strcmp(argv[argi], "--expires") == 0) {
            if (++argi >= argc || parse_u32(argv[argi], 1, UINT32_MAX, &expires) != 0) {
                usage(argv[0]);
                return (1);
            }
            argi++;
        ...
        if (strcmp(argv[argi], "--port") == 0) {
            if (++argi >= argc || parse_u32(argv[argi], 1, 65535, &port) != 0) {
                usage(argv[0]);
                return (1);
            }
            argi++;
        ...
        if (strcmp(argv[argi], "--timeout-ms") == 0) {
            if (++argi >= argc || parse_u32(argv[argi], 1, UINT32_MAX, &timeout_ms) != 0) {
                usage(argv[0]);
                return (1);
            }
            argi++;
    ...
    username = argv[argi++];
    password = argv[argi++];
    if (parse_target(server_ip, (uint16_t)port, &peer_ss, &peer_slen, &ctx.peer) != 0) {
        fprintf(stderr, "invalid server ip: %s\n", server_ip);
        return (1);
    }
    if (format_uri_host(uri_host, sizeof(uri_host), &ctx.peer) < 0) {
